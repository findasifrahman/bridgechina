<template>
  <div class="h-screen flex flex-col bg-slate-50">
    ...
    <div v-else-if="request" class="flex-1 overflow-y-auto p-4">
      ...
      <div class="bg-white rounded-lg p-4 mb-4">
        <div class="flex items-center gap-2 mb-3">
          <span class="px-3 py-1 bg-teal-100 text-teal-700 rounded-full text-sm font-medium">
            {{ request.category?.key || request.category?.name || 'Service' }}
          </span>
          <span
            :class="[
          ...
          <span v-if="request.sla_due_at" class="text-sm text-slate-500">
            SLA: {{ formatSlaTime(request.sla_due_at) }}
          </span>
        </div>

        <div class="text-sm text-slate-600 mb-2">
          <strong>City:</strong> {{ request.city?.name || 'N/A' }}
        </div>

        <div v-if="request.providerMessageContexts?.[0]" class="mb-4">
          ...
          <div class="text-sm text-slate-700 bg-slate-50 p-3 rounded">
            {{ request.providerMessageContexts[0].user_message_text }}
          </div>
          
          <div v-if="request.providerMessageContexts[0].extracted_summary" class="mt-3">
            <div class="text-sm font-medium text-slate-900 mb-2">Summary:</div>
            <div class="text-sm text-slate-700 bg-blue-50 p-3 rounded">
              {{ request.providerMessageContexts[0].extracted_summary }}
            </div>
          </div>
        </div>

        <div v-if="request.providerMessageContexts?.[0]?.extracted_payload" class="mb-4">
          <div class="text-sm font-medium text-slate-900 mb-2">AI Extracted Details:</div>
          <pre class="text-xs text-slate-600 bg-slate-50 p-3 rounded overflow-auto">{{ JSON.stringify(request.providerMessageContexts[0].extracted_payload, null, 2) }}</pre>
        </div>
      </div>

      <!-- Request Payload (Organized View) -->
      <div v-if="request.request_payload" class="bg-white rounded-lg p-4 mb-4">
        ...
        <div class="bg-slate-50 rounded-lg p-4 space-y-3">
          ...
          <template v-if="request.category?.key === 'shopping' && request.request_payload.items && Array.isArray(request.request_payload.items)">
            <div class="space-y-3">
              <div v-for="(item, idx) in request.request_payload.items" :key="idx" class="bg-white rounded-lg p-3 border border-slate-200">
                <div class="flex gap-3">
                  ...
                  <div class="flex-1">
                    ...
                    <div class="text-xs space-y-1">
                      ...
                      <div v-if="item.sourceUrl" class="mt-1">
                        <a :href="item.sourceUrl" target="_blank" class="text-teal-600 hover:underline text-xs break-all">
                          View Product Page →
                        </a>
                      </div>
                      <div v-if="item.weight" class="text-slate-500">
                        <span>Weight:</span> <span class="text-slate-700">{{ item.weight }}kg</span>
                      </div>
                      <div v-if="item.estimatedCost" class="text-slate-500">
                      ...
                      <div v-if="item.skuDetails && item.skuDetails.length > 0" class="mt-2 pt-2 border-t border-slate-200">
                        ...
                        <div v-for="(sku, skuIdx) in item.skuDetails" :key="skuIdx" class="text-xs text-slate-600 bg-slate-50 p-2 rounded mb-1">
                          <div><strong>{{ sku.sku?.props_names || `Option ${skuIdx + 1}` }}</strong></div>
                          <div>Qty: {{ sku.qty }}</div>
                          <div v-if="sku.sku?.sale_price">Price: ¥{{ sku.sku.sale_price }}</div>
              ...
              <div v-if="request.request_payload.estimatedTotalPrice || request.request_payload.totalItems" class="bg-white rounded-lg p-3 border border-slate-200 mt-2">
                <div class="flex justify-between text-sm">
                  <span class="text-slate-600">Total Items:</span>
                  <span class="font-semibold">{{ request.request_payload.totalItems || 0 }}</span>
                </div>
                <div v-if="request.request_payload.estimatedTotalPrice" class="flex justify-between text-sm mt-2 pt-2 border-t border-slate-200">
                  <span class="text-slate-600">Estimated Total:</span>
                  <span class="font-bold text-teal-600">¥{{ request.request_payload.estimatedTotalPrice }}</span>
                </div>
              </div>
            </div>
          </template>
          ...
          <template v-else-if="request.category?.key === 'shopping'">
            <div class="space-y-2 text-sm">
              <div v-if="request.request_payload.items_description">
                ...
                <p class="text-slate-900 mt-1">{{ request.request_payload.items_description }}</p>
              </div>
              <div v-if="request.request_payload.budget" class="flex gap-4">
                <div>
                  <span class="font-medium text-slate-700">Budget:</span>
                  <span class="text-slate-900 ml-2">¥{{ request.request_payload.budget }}</span>
                </div>
                <div v-if="request.request_payload.preferred_stores">
                  <span class="font-medium text-slate-700">Stores:</span>
                  <span class="text-slate-900 ml-2">{{ request.request_payload.preferred_stores }}</span>
                </div>
              </div>
            </div>
          </template>
            ...
            <div class="space-y-3 text-sm">
              <div class="grid grid-cols-2 gap-3">
                <div v-if="request.request_payload.hotel_name || request.request_payload.hotelName">
                  ...
                  <p class="text-slate-900">{{ request.request_payload.hotel_name || request.request_payload.hotelName }}</p>
                </div>
                <div v-if="request.request_payload.hotel_id || request.request_payload.hotelId">
                  <span class="font-medium text-slate-700">Hotel ID:</span>
                  <p class="text-slate-600 text-xs">{{ request.request_payload.hotel_id || request.request_payload.hotelId }}</p>
                </div>
                <div v-if="request.request_payload.check_in || request.request_payload.checkIn">
                  ...
                  <p class="text-slate-900">{{ request.request_payload.check_in || request.request_payload.checkIn }}</p>
                </div>
                <div v-if="request.request_payload.check_out || request.request_payload.checkOut">
                  ...
                  <p class="text-slate-900">{{ request.request_payload.check_out || request.request_payload.checkOut }}</p>
                </div>
                <div v-if="request.request_payload.guests">
                  <span class="font-medium text-slate-700">Guests:</span>
                  <p class="text-slate-900">{{ request.request_payload.guests }}</p>
                </div>
                <div v-if="request.request_payload.rooms || request.request_payload.room_qty">
                  <span class="font-medium text-slate-700">Rooms:</span>
                  <p class="text-slate-900">{{ request.request_payload.rooms || request.request_payload.room_qty }}</p>
                </div>
                <div v-if="request.request_payload.price || request.request_payload.total_price">
                  ...
                  <p class="text-teal-600 font-semibold">¥{{ request.request_payload.price || request.request_payload.total_price }}</p>
                </div>
              </div>
              <div v-if="request.request_payload.hotel_url || request.request_payload.booking_url || request.request_payload.sourceUrl" class="pt-2 border-t border-slate-200">
                <span class="font-medium text-slate-700">Booking URL:</span>
                <a 
                  :href="request.request_payload.hotel_url || request.request_payload.booking_url || request.request_payload.sourceUrl" 
                  target="_blank" 
                  class="text-teal-600 hover:underline text-xs break-all block mt-1"
                >
                  {{ request.request_payload.hotel_url || request.request_payload.booking_url || request.request_payload.sourceUrl }}
                </a>
              </div>
              <div v-if="request.request_payload.special_requests || request.request_payload.specialRequests" class="pt-2 border-t border-slate-200">
                <span class="font-medium text-slate-700">Special Requests:</span>
                <p class="text-slate-900 mt-1 whitespace-pre-wrap">{{ request.request_payload.special_requests || request.request_payload.specialRequests }}</p>
              </div>
            </div>
          </template>
          ...
          <template v-else-if="request.category?.key === 'transport'">
            <div class="grid grid-cols-2 gap-3 text-sm">
              <div v-if="request.request_payload.from_location || request.request_payload.fromLocation">
                ...
                <p class="text-slate-900">{{ request.request_payload.from_location || request.request_payload.fromLocation }}</p>
              </div>
              ...
              <div v-if="request.request_payload.vehicle_type || request.request_payload.vehicleType">
                ...
                <p class="text-slate-900">{{ request.request_payload.vehicle_type || request.request_payload.vehicleType }}</p>
              </div>
            </div>
          </template>
          ...
          <template v-else>
            <div class="space-y-2 text-sm">
              <div v-for="(value, key) in request.request_payload" :key="key">
      ...
      <!-- Payment Proof -->
      <div v-if="request.paymentProofs && request.paymentProofs.length > 0" class="bg-white rounded-lg p-4 mb-4">
        <div class="text-sm font-medium text-slate-900 mb-3">Payment Proof</div>
        <div class="space-y-3">
          <div
            v-for="proof in request.paymentProofs"
            :key="proof.id"
      ...
      <!-- Existing Offers -->
      <div v-if="request.providerOffers && request.providerOffers.length > 0" class="bg-white rounded-lg p-4 mb-4">
...
async function loadRequest() {
  ...
  try {
    ...
    // API returns { request: serviceRequest, dispatch }
